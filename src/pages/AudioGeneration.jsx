...
const AudioGeneration = () => {
    ...
    const [dialogueOptions] = useState({ stability: 0.5 });
    ...
    const playVoicePreview = (voiceId) => {
        ...
        if (voice && voice.sampleUrl) {
            ...
            alert("Превью для этого голоса недоступно");
        }
    };
    ...
    return (
        <>
            <Helmet>
                <title>Озвучка текста нейросетью и SFX эффекты | AI Asol</title>
                <meta name="description" content="Профессиональная озвучка текста голосом на русском языке с помощью ElevenLabs. Генерация звуковых эффектов (SFX) и диалогов нейросетью онлайн." />
            </Helmet>
                                            ...
                                            <div className="space-y-3">
                                                ...
                                                <div className="flex gap-2">
                                                    <select
                                                        ...
                                                        className="flex-1 bg-slate-50 border border-slate-100 rounded-2xl px-5 py-4 text-slate-700 font-bold focus:ring-4 focus:ring-indigo-500/5 focus:border-indigo-400 outline-none transition-all cursor-pointer"
                                                    >
                                                        {loadingVoices ? (
                                                    ...
                                                    <button
                                                        onClick={() => playVoicePreview(ttsVoice)}
                                                        disabled={!ttsVoice || loadingVoices}
                                                        className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all border disabled:opacity-50 ${playingVoice === ttsVoice ? 'bg-indigo-600 text-white border-indigo-600 shadow-md hover:bg-indigo-700' : 'bg-indigo-50 text-indigo-600 border-indigo-100 hover:bg-indigo-100'}`}
                                                        title={playingVoice === ttsVoice ? "Остановить" : "Послушать пример"}
                                                    >
                                                        {playingVoice === ttsVoice ? <Square size={16} fill="currentColor" /> : <Play size={20} fill="currentColor" />}
                                                    </button>
                                                </div>
                                                            ...
                                                            <div className="md:col-span-1">
                                                                ...
                                                                <div className="flex gap-2">
                                                                    <select
                                                                        ...
                                                                        className="flex-1 bg-white border border-slate-200 rounded-xl px-4 py-3 text-xs font-bold focus:border-emerald-500 outline-none transition-all"
                                                                    >
                                                                    ...
                                                                    <button
                                                                        onClick={() => playVoicePreview(block.voice)}
                                                                        disabled={!block.voice}
                                                                        className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all border disabled:opacity-50 ${playingVoice === block.voice ? 'bg-emerald-600 text-white border-emerald-600 shadow-sm hover:bg-emerald-700' : 'bg-emerald-50 text-emerald-600 border-emerald-100 hover:bg-emerald-100'}`}
                                                                        title={playingVoice === block.voice ? "Остановить" : "Послушать пример"}
                                                                    >
                                                                        {playingVoice === block.voice ? <Square size={12} fill="currentColor" /> : <Play size={16} fill="currentColor" />}
                                                                    </button>
                                                                </div>
                                                            </div>
